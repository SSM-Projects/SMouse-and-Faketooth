...
_In_ WDFDEVICE  Device)
{
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_QueueInitialize\n");
#endif
	...
	ioQueueConfig.EvtIoDefault = FakeKeyboard_EvtIoDefault;
	ioQueueConfig.EvtIoInternalDeviceControl = FakeKeyboard_EvtIoInternalDeviceControl;

	ntStatus = WdfIoQueueCreate(Device, &ioQueueConfig, WDF_NO_OBJECT_ATTRIBUTES, &devContext->IoMouseQueue);	// Mouse Queue

	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Mouse Queue Create failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	ntStatus = WdfIoQueueCreate(Device, &ioQueueConfig, WDF_NO_OBJECT_ATTRIBUTES, &devContext->ReadMsgQueue);	// readMsg Queue

	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Interrupt Msg Queue Create failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	ntStatus = WdfIoQueueCreate(Device, &ioQueueConfig, WDF_NO_OBJECT_ATTRIBUTES, &devContext->WriteMsgQueue);	// WriteMsg Queue
	...
	ioQueueConfig.EvtIoRead = FakeKeyboard_EvtIoRead;
	ioQueueConfig.EvtIoStop = FakeKeyboard_EvtIoStop;

	ntStatus = WdfIoQueueCreate(Device, &ioQueueConfig, WDF_NO_OBJECT_ATTRIBUTES, &devContext->IoReadQueue);	// read Queue

	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Read Queue Create failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus)) {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Read Queue setting failed 0x%x\n", ntStatus);
		return ntStatus;
	}
	...
	ioQueueConfig.EvtIoWrite = FakeKeyboard_EvtIoWrite;
	ioQueueConfig.EvtIoStop = FakeKeyboard_EvtIoStop;
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - write Queue Create failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - write Queue setting failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
...
VOID
FakeKeyboard_EvtIoDefault(
_In_ WDFQUEUE Queue,
_In_ WDFREQUEST Request)
{
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtIoDefault Queue 0x%p, Request 0x%p", Queue, Request);
#else
...
VOID
FakeKeyboard_EvtIoStop(
_In_ WDFQUEUE Queue,
_In_ WDFREQUEST Request,
_In_ ULONG ActionFlags)
{
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtIoStop, Queue 0x%p, Request 0x%p ActionFlags %d",
...
_In_ size_t Length)
{
	NTSTATUS ntStatus;
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtIoRead Start\n");
#endif
	// 유효성 검사 
	if (Length > TEST_BOARD_TRANSFER_BUFFER_SIZE) {
		ntStatus = STATUS_INVALID_PARAMETER;
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - STATUS_INVALID_PARAMETER failed 0x%x === \n", ntStatus);
#endif
		goto Exit;
	}
...
_In_ size_t Length)
{
	NTSTATUS ntStatus;
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtIoWrite Start\n");
#endif
	// 유효성 검사 
	if (Length > TEST_BOARD_TRANSFER_BUFFER_SIZE) {
		ntStatus = STATUS_INVALID_PARAMETER;
	...
	BOOLEAN is_ioctl)
{
	...
	WDFUSBPIPE Pipe;
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - CommonBulkReadWrite Start\n");
#endif
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - CommonBulkReadWrite: WdfRequestGetStatus (is_read = %u) failed: %08X\n", is_read, ntStatus);
#endif
	WdfRequestCompleteWithInformation(Request, ntStatus, 0);
...
{
	FakeKeyboard_EvtRequestCompletionRoutine(Request, Target, CompletionParams, Context);
	return;
}
...
*/
VOID
FakeKeyboard_EvtRequestCompletionRoutine(
_In_ WDFREQUEST Request,
_In_ WDFIOTARGET Target,
PWDF_REQUEST_COMPLETION_PARAMS CompletionParams,
_In_ WDFCONTEXT Context)
{
	...
	if (!NT_SUCCESS(ntStatus)){
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Request completed with failure: %X\n", ntStatus);
#endif
...
VOID
FakeKeyboard_EvtIoInternalDeviceControl(
_In_ WDFQUEUE Queue,
_In_ WDFREQUEST Request,
...
_In_ ULONG IoControlCode)
{
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - %s, Queue:0x%p, Request:0x%p\n", DbgHidInternalIoctlString(IoControlCode), Queue, Request);
#endif
	...
	case IOCTL_HID_GET_DEVICE_DESCRIPTOR:
		...
		//
		ntStatus = FakeKeyboard_GetHidDescriptor(Device, Request);
		break;
	...
	case IOCTL_HID_GET_DEVICE_ATTRIBUTES:
		...
		//
		ntStatus = FakeKeyboard_GetDeviceAttributes(Request);
		break;
	...
	case IOCTL_HID_GET_REPORT_DESCRIPTOR:
		...
		//
		ntStatus = FakeKeyboard_GetReportDescriptor(Device, Request);
		break;
	...
	case IOCTL_HID_READ_REPORT:
		...
		ntStatus = WdfRequestForwardToIoQueue(Request, devContext->ReadMsgQueue);

		if (!NT_SUCCESS(ntStatus)){
#ifdef DBGs
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfRequestForwardToIoQueue failed with ntStatus: 0x%x\n", ntStatus);
#endif
			WdfRequestComplete(Request, ntStatus);
		}
	...
	case IOCTL_HID_SEND_IDLE_NOTIFICATION_REQUEST:
		...
		//
		//if (!NT_SUCCESS(ntStatus)) {
		//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - SendIdleNotification failed with ntStatus: 0x%x\n", ntStatus);
		//
		//	WdfRequestComplete(Request, ntStatus);
		//}
		//
	...
	case IOCTL_HID_WRITE_REPORT:
		...
		// write report를 여기서 처리 해줘야 하나?? 
		// 일단 Queue에 저장함 

		ntStatus = WdfRequestForwardToIoQueue(Request, devContext->WriteMsgQueue);
	...
	case IOCTL_HID_GET_STRING:
		//
		// Requests that the Keyboard Driver retrieve a human-readable string
	...
	default:
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - IOCTL_Default CALL\n");
#endif
...
IN WDFREQUEST Request)
{
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetHidDescriptor Entry\n");
#endif
	//
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfRequestRetrieveOutputMemory failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
...
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - G_DefaultHidDescriptor is zero, 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfMemoryCopyFromBuffer failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetHidDescriptor Exit = 0x%x\n", ntStatus);
#endif
	return ntStatus;
}

NTSTATUS
FakeKeyboard_GetDeviceAttributes(
IN WDFREQUEST Request)
{
	NTSTATUS                 ntStatus = STATUS_SUCCESS;
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetDeviceAttributes Entry\n");
#endif	
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfRequestRetrieveOutputBuffer failed 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetDeviceAttributes Exit = 0x%x\n", ntStatus);
#endif
	return ntStatus;
}
...
NTSTATUS
FakeKeyboard_GetReportDescriptor(
IN WDFDEVICE Device,
IN WDFREQUEST Request)
{
	NTSTATUS            ntStatus = STATUS_SUCCESS;
	...
	UNREFERENCED_PARAMETER(Device);
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetReportDescriptor Entry\n");
#endif
	//
	// This IOCTL is METHOD_NEITHER so WdfRequestRetrieveOutputMemory
	// will correctly retrieve buffer from Irp->UserBuffer. 
	...
	if (bytesToCopy == 0) {
		ntStatus = STATUS_INVALID_DEVICE_STATE;
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - G_DefaultHidDescriptor's reportLenght is zero, 0x%x\n", ntStatus);
#endif
		return ntStatus;
	}
	...
	WdfRequestSetInformation(Request, bytesToCopy);
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_GetReportDescriptor Exit = 0x%x\n", ntStatus);
#endif
	return ntStatus;
}
...
//	PAGED_CODE();
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HidFx2SetFeature Enter\n");
//
...
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Userbuffer is small 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
//	//
...
//		ntStatus = STATUS_INVALID_DEVICE_REQUEST;
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Irp->UserBuffer is NULL 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
//	if (transferPacket->reportBufferLen == 0){
//		ntStatus = STATUS_BUFFER_TOO_SMALL;
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HID_XFER_PACKET->reportBufferLen is 0, 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
//	if (transferPacket->reportBufferLen < sizeof(UCHAR)){
//		ntStatus = STATUS_BUFFER_TOO_SMALL;
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HID_XFER_PACKET->reportBufferLen is too small, 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
...
//	{
//		ntStatus = STATUS_INVALID_DEVICE_REQUEST;
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Incorrect report ID, 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HidFx2SetFeature Exit\n");
//	return ntStatus;
//}
//
//NTSTATUS
...
//	PAGED_CODE();
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HidFx2GetFeature Enter\n");
//
//	*BytesReturned = 0;
//
//	device = WdfIoQueueGetDevice(WdfRequestGetIoQueue(Request));
...
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - HidFx2GetFeature Exit\n");
//	return ntStatus;
//}
//
//NTSTATUS
...
//	PAGED_CODE();
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - SendVendorCommand Enter\n");
//
//	pDevContext = GetDeviceContext(Device);
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - SendVendorCommand: Command:0x%x, data: 0x%x\n",
...
//
//	if (!NT_SUCCESS(ntStatus)) {
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - SendtVendorCommand: Failed to set Segment Display state - 0x%x \n",
//			ntStatus);
//	}
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - SendVendorCommand Exit\n");
//
//	return ntStatus;
//}
//
...
//	PAGED_CODE();
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - GetVendorData Enter\n");
//
//	pDevContext = GetDeviceContext(Device);
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - GetVendorData: Command:0x%x, data: 0x%x\n",
//		VendorCommand, *CommandData);
//
//	//
...
//
//	if (!NT_SUCCESS(ntStatus)) {
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - GetVendorData: Failed to get state - 0x%x \n",
//			ntStatus);
//	}
//	else
//	{
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - GetVendorData: Command:0x%x, data after command: 0x%x\n",
//			VendorCommand, *CommandData);
//	}
//
//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - GetVendorData Exit\n");
//
//	return ntStatus;
//}
//
...
//
//		ntStatus = STATUS_BUFFER_TOO_SMALL;
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - DeviceIoControl.InputBufferLength too small, 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
...
//		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Incorrect DeviceIoControl.InputBufferLength, 0x%x\n", ntStatus);
//		return ntStatus;
//	}
//
//	//
