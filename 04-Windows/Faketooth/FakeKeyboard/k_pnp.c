...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, FakeKeyboard_EvtDevicePrepareHardware)
#pragma alloc_text(PAGE, FakeKeyboard_EvtDeviceD0Exit)

#endif

NTSTATUS
FakeKeyboard_EvtDevicePrepareHardware(
IN WDFDEVICE    Device,
...
IN WDFCMRESLIST ResourceListTranslated)
{
	...
	PAGED_CODE();
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Evt Device Prepare Hardware === \n");
#endif
...
#ifdef DBGs
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfUsbTargetDeviceCreate Failed 0x%x === \n", ntStatus);
#endif
			return ntStatus;
		}
	}
	...
	FakeKeyboard_InitializePipeInformation(Device);

	FakeKeyboard_GetConfigurationDescriptor(Device);

	// power Management Init
	FakeKeyboard_InitPowerManagement(Device);

	// Continuous Reader
	FakeKeyboard_ConfigureContinuousReader(Device);

	return ntStatus;
}

NTSTATUS
FakeKeyboard_EvtDeviceReleaseHardware(
...
_In_ WDFCMRESLIST ResourceListTranslated)
{
	...
	UNREFERENCED_PARAMETER(ResourceListTranslated);
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Evt Device Release Hardware === \n");
#endif
...
NTSTATUS
FakeKeyboard_EvtDeviceSurpriseRemoval(
_In_ WDFDEVICE Device)
{
	PAGED_CODE();
	UNREFERENCED_PARAMETER(Device);
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Evt Device Surprise Removal === \n");
#endif
...
*/
NTSTATUS
FakeKeyboard_InitializePipeInformation(
_In_ WDFDEVICE Device)
{
	NTSTATUS								ntStatus;
	PDEVICE_EXTENSION						devContext;
	...
	UNREFERENCED_PARAMETER(Device);
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Initialize Pipe Information === \n");
#endif
	devContext = GetDeviceContext(Device);
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfUsbTargetDeviceSelectConfig Failed 0x%x === \n", ntStatus);
#endif
		return ntStatus;
	}
	...
	//ntStatus = WdfUsbTargetDeviceSelectConfig(devContext->UsbDevice, WDF_NO_OBJECT_ATTRIBUTES, &configParams);
	//if (!NT_SUCCESS(ntStatus)) {
	//	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfUsbTargetDeviceSelectConfig failed, ntStatus : 0x%x\n", ntStatus);
	//	return ntStatus;
	//}
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - WdfMemoryCreate for Device Descriptor failed %!STATUS!\n", ntStatus);
#endif
		return ntStatus;
	}
...
#ifdef DBGs
				DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
					"=== Keyboard Driver - Interrupt In, type:%d, EP:0x%x, Pipe:%p, MaximumPacketSize : %d === \n", pipeInformation.PipeType, pipeInformation.EndpointAddress, pipe, pipeInformation.MaximumPacketSize);
#endif
			}
			else {
...
#ifdef DBGs
				DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
					"=== Keyboard Driver - Interrupt Out, type:%d, EP:0x%x, Pipe:%p === \n", pipeInformation.PipeType, pipeInformation.EndpointAddress, pipe);
#endif
			}
...
*/
NTSTATUS
FakeKeyboard_GetConfigurationDescriptor(
_In_ WDFDEVICE Device)
{
	NTSTATUS ntStatus = STATUS_SUCCESS;
	//	WDF_OBJECT_ATTRIBUTES attributes;
	...
	//	
	//DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Get Configuration Descriptor st=%x, bConfiguratopnValue=%d === \n",ntStatus,devContext->ConfigurationDescriptor->bConfigurationValue);
...
NTSTATUS
FakeKeyboard_InitPowerManagement(
_In_ WDFDEVICE  Device)
{
	NTSTATUS ntStatus = STATUS_SUCCESS;
	PDEVICE_EXTENSION devContext;
...
#ifdef DBGs	
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Device init power management\n");
#endif
...
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
			"=== Keyboard Driver - WdfUsbTargetDeviceRetrieveInformation failed with ntStatus 0x%08x\n", ntStatus);
#endif
		return ntStatus;
	}

#ifdef DBGs		
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Device self powered: %d\n",
		usbInfo.Traits & WDF_USB_DEVICE_TRAIT_SELF_POWERED ? 1 : 0);
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Device remote wake capable: %d\n",
		usbInfo.Traits & WDF_USB_DEVICE_TRAIT_REMOTE_WAKE_CAPABLE ? 1 : 0);
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Device high speed: %d\n",
...
#ifdef DBGs
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
				"=== Keyboard Driver - WdfDeviceAssignS0IdleSettings failed with ntStatus 0x%08x\n", ntStatus);
#endif
			return ntStatus;
		}
...
#ifdef DBGs
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
				"=== Keyboard Driver - WdfDeviceAssignSxWakeSettings failed with ntStatus 0x%08x\n", ntStatus);
#endif
			return ntStatus;
		}
	}
...
IN  WDF_POWER_DEVICE_STATE PreviousState)
{
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtDeviceD0Entry Enter - coming from %s\n", DbgDevicePowerString(PreviousState));
#endif
	...
	if (!NT_SUCCESS(ntStatus)) {
#ifdef DBGs
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - Could not start Bulk pipe failed 0x%x\n", ntStatus);
#endif
	}
...
#ifdef DBGs
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "=== Keyboard Driver - FakeKeyboard_EvtDeviceD0Exit Enter- moving to %s\n", DbgDevicePowerString(TargetState));
#else
