...
VOID SPM_EvtRequestCompletionRoutine(_In_ WDFREQUEST Request, _In_ WDFIOTARGET Target, PWDF_REQUEST_COMPLETION_PARAMS CompletionParams, _In_ WDFCONTEXT Context);
...
VOID SPM_DispatchPassThrough(_In_ WDFREQUEST Request, _In_ WDFIOTARGET Target);
PCHAR DbgHidInternalIoctlString(IN ULONG IoControlCode);

NTSTATUS SPM_GetHidDescriptor(IN WDFDEVICE Device, IN WDFREQUEST Request);
NTSTATUS SPM_GetDeviceAttributes(IN WDFREQUEST Request);
NTSTATUS SPM_GetReportDescriptor(IN WDFDEVICE Device, IN WDFREQUEST Request);
