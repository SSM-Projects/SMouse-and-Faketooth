...
public class UsbDeviceManager {
	...
	private static final int MSG_ENABLE_MOUSE = 6;
	...
	private class MouseSettingsObserver extends ContentObserver {
		public MouseSettingsObserver() {
			super(null);
		}
		@Override
		public void onChange(boolean selfChange) {
			boolean enable = (Settings.Global.getInt(mContentResolver,
					Settings.Global.MOUSE_MODE_ON, 0) > 0);
			mHandler.sendMessage(MSG_ENABLE_MOUSE, enable);
		}
	}
    ...
    public void systemReady() {
        ...
        // make sure the MOUSE_MODE_ON setting value matches the current state
		Settings.Global.putInt(mContentResolver, Settings.Global.MOUSE_MODE_ON, mMouseEnabled ? 1 : 0);
    ...
    private final class UsbHandler extends Handler {
        ...
        private void setMouseEnabled(boolean enable) {
            if (DEBUG) Slog.d(TAG, "setMouseEnabled: " + enable);
            if (enable != mMouseEnabled) {
                mMouseEnabled = enable;
                // Due to the persist.sys.usb.config property trigger, changing mouse state requires
                // switching to default function
                setEnabledFunctions(mDefaultFunctions, true);
            }
        }
        ...
        private void setEnabledFunctions(String functions, boolean makeDefault) {
            ...
            if (functions != null && makeDefault && !needsOemUsbOverride()) {
				...
				if (mMouseEnabled) {
					functions = addFunction(functions, UsbManager.USB_FUNCTION_MOUSE);
				} else {
					functions = removeFunction(functions, UsbManager.USB_FUNCTION_MOUSE);
				}
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_ENABLE_ADB:
                    ...
                    break;
				case MSG_ENABLE_MOUSE:
					setMouseEnabled(msg.arg1 == 1);
					break;
