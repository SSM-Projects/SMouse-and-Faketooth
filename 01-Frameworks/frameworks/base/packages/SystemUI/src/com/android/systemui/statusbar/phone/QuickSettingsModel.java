...
import com.android.systemui.statusbar.policy.MouseController.MouseStateChangeCallback;
...
class QuickSettingsModel implements BluetoothStateChangeCallback,
    ...
    private QuickSettingsTileView mMouseModeTile;
    private RefreshCallback mMouseModeCallback;
    private State mMouseModeState = new State();
	...
	void addMouseModeTile(QuickSettingsTileView view, RefreshCallback cb) {
		mMouseModeTile = view;
		mMouseModeTile.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				if (mMouseModeState.enabled) {
					setMouseModeState(false);
				} else {
					setMouseModeState(true);
				}
				int mouseMode = Settings.Global.getInt(mContext.getContentResolver(),
						Settings.Global.MOUSE_MODE_ON, 0);
				onMouseModeChanged(mouseMode != 0);
			}
		});
		mMouseModeCallback = cb;
		int mouseMode = Settings.Global.getInt(mContext.getContentResolver(),
				Settings.Global.MOUSE_MODE_ON, 0);
		onMouseModeChanged(mouseMode != 0);
	}
	private void setMouseModeState(boolean enabled) {
		// Change the system setting
        Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.MOUSE_MODE_ON,
                                enabled ? 1 : 0);
    }
    @Override
    public void onMouseModeChanged(boolean enabled) {
        Resources r = mContext.getResources();
        mMouseModeState.enabled = enabled;
        mMouseModeState.iconId = (enabled ?
                R.drawable.ic_qs_mouse_on :
                R.drawable.ic_qs_mouse_off);
        mMouseModeState.label = r.getString(R.string.quick_settings_mouse_mode_label);
        mMouseModeCallback.refreshView(mMouseModeTile, mMouseModeState);
    }
