...
enum {
    ...
    SET_BLUETOOTH_SELECTED_HOST // Faketooth
};
...
public:
    ...
    // Faketooth
    virtual int32_t setBluetoothSelectedHost(int selectedHost)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        data.writeInt32(selectedHost);
        remote()->transact(SET_BLUETOOTH_SELECTED_HOST, data, &reply);
        return reply.readInt32();
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        // Faketooth
        case SET_BLUETOOTH_SELECTED_HOST: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            int32_t selectedHost = (int32_t)data.readInt32();
            reply->writeInt32( setBluetoothSelectedHost(selectedHost) );
            return NO_ERROR;
        }
