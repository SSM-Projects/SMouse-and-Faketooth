...
// Faketooth
int32_t AudioPolicyService::setBluetoothSelectedHost(int selectedHost)
{
    if (mpAudioPolicy == NULL) {
        ALOGV("mpAudioPolicy == NULL");
        return 0;
    ...
    if (mpAudioPolicy->set_bluetooth_selected_host == NULL) {
        ALOGV("HAL does not implement set_bluetooth_selected_host");
        return 0;
    }

    return mpAudioPolicy->set_bluetooth_selected_host(mpAudioPolicy, selectedHost);
}
